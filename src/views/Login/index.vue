<template>
  <div class="login_form_page_container">
    ...
    <div>
      <el-image style="width: 614px; height:734px;" :src="filter.getFile('Maskgroup.png')" fit="cover" />
    </div>
  </div>
</template>
...
import { setStorage, setCookieItem, getCookieItem } from '@/utils/common'
...
onMounted(() => {
  let user_pwd = getCookieItem('user_pwd')
  if (user_pwd) {
    checked.value = true
    modelRef = JSON.parse(user_pwd)
  } else {
    checked.value = false
  }
})
